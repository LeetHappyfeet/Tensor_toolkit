...
class TensorToolkitGUI:
    ...
    def _build_controls(self):
        ...
        canvas = tk.Canvas(outer, width=340, highlightthickness=0)
        ...
        canvas.create_window((0, 0), window=panel, anchor="nw", width=325)
        ...
        ttk.Entry(grid_frame, textvariable=self.points_var, width=12).grid(row=0, column=1, sticky="e")
        ...
        ttk.Label(grid_frame, textvariable=self.grid_summary_var, wraplength=285).grid(
        ...
        for i, name in enumerate(GUI_OUTPUTS):
            ...
            ttk.Checkbutton(
                output_frame, text=name, variable=var, command=self._update_resource_estimate
        ...
        ttk.Combobox(
            execution, textvariable=self.memory_mode_var,
            values=("auto", "in_memory", "tiled"), state="readonly", width=13
        ).grid(row=0, column=1, sticky="e")
        ttk.Label(execution, text="Time-core hint").grid(row=1, column=0, sticky="w")
        self.tile_points_var = tk.StringVar(value="8")
        ttk.Entry(execution, textvariable=self.tile_points_var, width=13).grid(row=1, column=1, sticky="e")
        ttk.Label(
            execution,
            text="Spatial block sizes are chosen automatically from the RAM budget.",
            wraplength=285,
        ).grid(row=2, column=0, columnspan=2, sticky="w", pady=(2, 4))
        ttk.Label(execution, text="Output storage").grid(row=3, column=0, sticky="w")
        self.storage_mode_var = tk.StringVar(value="auto")
        ttk.Combobox(
            execution, textvariable=self.storage_mode_var,
            values=("auto", "memory", "disk"), state="readonly", width=13
        ).grid(row=3, column=1, sticky="e")
        ttk.Label(execution, text="Result directory").grid(row=4, column=0, sticky="w")
        self.output_path_var = tk.StringVar(value=str(Path("results") / "gui_run"))
        ttk.Entry(execution, textvariable=self.output_path_var, width=20).grid(row=4, column=1, sticky="e")
        ttk.Button(execution, text="Choose…", command=self._choose_output_dir).grid(
            row=5, column=1, sticky="e", pady=(3, 0)
        )
        self.resource_var = tk.StringVar(value="")
        ttk.Label(execution, textvariable=self.resource_var, wraplength=285, justify="left").grid(
            row=6, column=0, columnspan=2, sticky="w", pady=(6, 0)
        )
        ttk.Button(execution, text="Estimate resources", command=self._update_resource_estimate).grid(
        ...
        ttk.Button(buttons, text="Open result", command=self._open_result).grid(
        ...
        mu_box.grid(row=row, column=1, sticky="w")
        mu_box.bind("<<ComboboxSelected>>", lambda _e: self._refresh_view()); row += 1
        ...
        nu_box.grid(row=row, column=1, sticky="w")
        nu_box.bind("<<ComboboxSelected>>", lambda _e: self._refresh_view()); row += 1
        self.component_label_var = tk.StringVar(value="Selected: g_tt [0,0]")
        ttk.Label(panel, textvariable=self.component_label_var, font=("TkDefaultFont", 9, "bold")).grid(
        ...
        hbox.grid(row=row, column=1, sticky="w")
        hbox.bind("<<ComboboxSelected>>", lambda _e: self._axes_changed()); row += 1
        ...
        vbox.grid(row=row, column=1, sticky="w")
        vbox.bind("<<ComboboxSelected>>", lambda _e: self._axes_changed()); row += 1
        ...
        ttk.Label(panel, textvariable=self.status_var, wraplength=290).grid(
    ...
    def _build_workspace(self):
        ...
        for tab, attr, initial in (
            (details_tab, "center_text", "Run a simulation to inspect the center tensor.\n"),
            ...
            widget.insert("1.0", initial); widget.configure(state="disabled")
    ...
    def _build_experiment(self):
        ...
        if tile_points < 1:
            raise ValueError("time-core hint must be at least 1")
    ...
    def _resource_plan(self):
        points = int(self.points_var.get())
        tile = int(self.tile_points_var.get())
        if points < 3 or tile < 1:
            raise ValueError("points must be >=3 and time-core hint >=1")
        outputs = self._selected_outputs()
        shape = (points,) * 4
        path_text = self.output_path_var.get().strip()
        path = Path(path_text) if path_text else None
        storage = select_storage_mode(
            shape, outputs, requested_mode=self.storage_mode_var.get(),
            output_path=path, limit_fraction=0.65,
        )
        plan = memory_plan(
            shape, outputs, requested_mode=self.memory_mode_var.get(),
            tile_points=tile, limit_fraction=0.65, disk_backed=storage == "disk",
        )
        return outputs, shape, path, storage, plan

    def _update_resource_estimate(self):
        try:
            outputs, shape, path, storage, plan = self._resource_plan()
            points = shape[0]
            persistent = output_bytes(shape, outputs)
            free_disk = None
            if path is not None:
                probe = path if path.exists() else next(
                    (p for p in (path, *path.parents) if p.exists()), Path.cwd()
                )
                free_disk = shutil.disk_usage(probe).free
            spacing = (2 * float(self.extent_var.get())) / (points - 1)
            self.grid_summary_var.set(
                f"{points**4:,} spacetime points; spacing Δ≈{spacing:g}"
            )
            block = plan.get("block_shape", shape)
            local = plan.get("max_local_shape", shape)
            self.resource_var.set(
                f"Plan: {plan['selected_mode']} + {storage}\n"
                f"Core block t×x×y×z: {_shape_text(block)}\n"
                f"Max with halos: {_shape_text(local)}\n"
                f"Blocks: {plan.get('block_count', 1):,}\n"
    ...
    def _start_run(self):
        try:
            ...
            _outputs, _shape, output_path, storage_mode, plan = self._resource_plan()
        ...
        block = _shape_text(plan.get("block_shape", ()))
        self.status_var.set(f"Calculating on CPU with {block} core blocks…")
    ...
    def _run_finished(self, result, output_path=None):
        ...
        self.status_var.set(
            f"Complete: {result.metric_name}; block {_shape_text(memory.get('block_shape', ())) }; "
            f"validation {result.metadata.get('diagnostics', {}).get('status', 'UNKNOWN')}{suffix}"
        )
    ...
    def _populate_result_controls(self):
        names = sorted(
            name for name, value in self.fields.items()
    ...
    def _build_fixed_controls(self):
        ...
        for row, axis in enumerate(a for a in range(4) if a not in (horizontal, vertical)):
            ...
            spin = self.ttk.Spinbox(
                self.fixed_frame, from_=0, to=len(values)-1, textvariable=variable, width=6,
                command=lambda a=axis, v=variable, l=value_label: self._fixed_changed(a, v, l),
            )
            spin.grid(row=row, column=1, sticky="e")
            self.ttk.Label(self.fixed_frame, textvariable=value_label).grid(
                row=row, column=2, sticky="w", padx=(5, 0)
            )

    def _fixed_changed(self, axis, variable, label):
        try:
            label.set(f"{COORDINATE_NAMES[axis]}={self.axis_values[axis][int(variable.get())]:g}")
        except Exception:
            return
        self._refresh_view()
    ...
    def _refresh_view(self):
        ...
        try:
            ...
            data = extract_2d_slice(
                field, component=(mu, nu), horizontal_axis=horizontal,
                vertical_axis=vertical, fixed_indices=fixed,
            )
        except (ValueError, IndexError) as exc:
            self.status_var.set(f"View error: {exc}"); return

        label = tensor_component_label(field_name, mu, nu)
        self.component_label_var.set(f"Selected: {label}")
        ...
        fixed_desc = ", ".join(
            f"{COORDINATE_NAMES[a]}={self.axis_values[a][i]:g}" for a, i in sorted(fixed.items())
        )
    ...
    def _refresh_overview(self, field_name, horizontal, vertical, fixed):
        ...
        for mu in range(4):
            for nu in range(4):
                ax = axes[mu, nu]
                mesh = ax.pcolormesh(
                    x_values, y_values, slices[mu][nu], shading="auto",
                    vmin=-vmax, vmax=vmax, cmap="coolwarm",
                )
                ax.set_title(tensor_component_label(field_name, mu, nu).split(" [", 1)[0], fontsize=8)
                if mu == 3:
                    ax.set_xlabel(self.horizontal_var.get(), fontsize=7)
                if nu == 0:
                    ax.set_ylabel(self.vertical_var.get(), fontsize=7)
                ax.tick_params(labelsize=6)
        if mesh is not None:
            self.overview_figure.colorbar(mesh, ax=axes.ravel().tolist(), shrink=0.7, label=field_name)
        self.overview_figure.suptitle(f"{self.metric_name}: {field_name} 4×4 component overview")
        self.overview_figure.subplots_adjust(
            left=0.07, right=0.88, bottom=0.07, top=0.91, wspace=0.28, hspace=0.38
        )
    ...
    def _refresh_center_text(self):
        ...
        self.center_text.delete("1.0", "end"); self.center_text.insert("1.0", text)
    ...
    def _refresh_validation(self):
        ...
        lines = [
            ...
            f"Core block: {_shape_text(memory.get('block_shape', ())) }",
            f"Maximum halo block: {_shape_text(memory.get('max_local_shape', ())) }",
            f"Block count: {memory.get('block_count', 'unknown')}",
            f"Storage: {storage.get('mode', 'unknown')} ({storage.get('format', 'unknown')})",
            "",
        ]
        for name, item in diagnostics.get("fields", {}).items():
            lines += [
                name,
                f"  finite: {item.get('finite', 'unknown')}",
                f"  max |value|: {item.get('max_abs', 0):.10g}",
            ]
            symmetry = item.get("symmetry")
            if symmetry:
                lines += [
                    f"  symmetry absolute residual: {symmetry['absolute']:.10g}",
                    f"  symmetry relative residual: {symmetry['relative']:.10g}",
                ]
            lines.append("")
        lines += [
            "PASS means the implemented numerical checks passed.",
            "WARNING means a numerical residual exceeded the configured threshold.",
        ]
    ...
    def _open_result(self):
        ...
        self.result = None
        self.metadata, self.fields, self.axis_values = metadata, fields, axes
